...
def evaluate(model):
    ...
    print("本次预测集中共有%d样本：%d个1类样本，%d个2类样本，%d个3类样本" % (y.shape[0], sum(y[:, 0] > 0), sum(y[:, 1] > 0), sum(y[:, 2] > 0)))
    correct, wrong = 0, 0
    with torch.no_grad():
        y_pred = model(x)  # 模型预测
        _, y_pred_cls = torch.max(y_pred, 1)
        for y_p, y_t in zip(y_pred_cls, y_cls):  # 与真实标签进行对比
            if y_p == y_t:
                correct += 1  # 正样本判断正确
            else:
                wrong += 1
...
def main():
    ...
    train_sample = 10_0000  # 每轮训练总共训练的样本总数
    input_size = 5  # 输入向量维度
