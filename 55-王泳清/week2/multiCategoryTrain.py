...
class TorchModel(nn.Module):
    def __init__(self, input_size, hidden_size):
        ...
        self.linear_1 = nn.Linear(input_size, hidden_size)  # 线性层1
        self.linear_2 = nn.Linear(hidden_size, 1)  # 线性层2
        # self.linear = nn.Linear(input_size, 1)  # 线性层
        # self.activation = torch.relu # 激活函数
        self.activation = torch.sigmoid # 激活函数
        # self.loss = nn.functional.l1_loss  # 损失函数
    ...
    with torch.no_grad():
        ...
        for y_p, y_t in zip(y_pred, y):  # 与真实标签进行对比
            if float(y_p) < 0.5 and int(y_t) == 0:
                ...
                correct += 1  # 正样本判断正确
            # elif float(y_p) < 0.6 and float(y_p) >= 0.4:
            #     continue
            else:
                wrong += 1
