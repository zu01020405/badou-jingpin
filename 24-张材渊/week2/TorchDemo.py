...
def build_sample():
    ...
    if x[0] > x[4] and x[0] > x[1]:
        return x, [0, 1, 0]
    elif x[0] > x[4] and x[0] <= x[1]:
        return x, [0, 0, 1]
    else:
        return x, [1, 0, 0]
...
def evaluate(model):
    ...
    with torch.no_grad():
        ...
        for y_p, y_t in zip(y_pred, y):  # 与真实标签进行对比
            if float(y_p[0]) > 0.5 and list(y_t) == [1, 0, 0]:
                correct += 1  # 负样本判断正确
            elif float(y_p[1]) > 0.5 and list(y_t) == [0, 1, 0]:
                correct += 1  # 正样本判断正确
            elif float(y_p[2]) > 0.5 and list(y_t) == [0, 0, 1]:
                print(y_p, y_t)
                correct += 1  # 正样本判断正确
            else:
                wrong += 1
...
def predict(model_path, input_vec):
    ...
    for vec, res in zip(input_vec, result):
        print("输入：%s,  概率值：%s" % (vec, list(res)))  # 打印结果
...
if __name__ == "__main__":
    ...
    test_vec = [[0.47889086, 0.15229675, 0.31082123, 0.03504317, 0.18920843],
                ...
                [0.79349776, 0.99416669, 0.92579291, 0.41567412, 0.3358894]]
