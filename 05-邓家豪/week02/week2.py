...
def evaluate(model):
    ...
    with torch.no_grad():
        ...
        for y_p, y_t in zip(y_pred, y):  # 与真实标签进行对比
            if float(y_p) < 1.0 / 3.0 and y_t == 0.3:
                correct += 1  # 样本1判断正确
            elif float(y_p) > 2.0 / 3.0 and y_t == 0.6:
                correct += 1  # 样本2判断正确
            elif 1.0 / 3.0 <= float(y_p) <= 2.0 / 3.0 and y_t == 0.9:
                correct += 1  # 样本3判断正确
            else:
                wrong += 1
...
def predict(model_path, input_vec):
    ...
    for vec, res in zip(input_vec, result):
        print("输入：%s, 预测类别：%d, 预测概率值：%f" % (vec, pre_type(res), res))  # 打印结果


if __name__ == "__main__":
    main()
    test_vec = [[0.47889086, 0.15229675, 0.31082123, 0.03504317, 0.18920843],
                [0.04963533, 0.5524256, 0.65758807, 0.95520434, 0.84890681],
