...
规律: x是一个5维向量: 取向量中最大的那个元素的index作为类别(因此此问题为5分类问题)
...
# 随机生成一个5维向量， 取向量中最大的那个元素的index作为类别(因此此问题为5分类问题)
def build_sample():
    x = np.random.randn(5)
    return x, x.argmax()
...
def build_dataset(total_sample_num):
    ...
    for _ in range(total_sample_num):
        ...
        X.append(x)
        Y.append(y)
    return torch.FloatTensor(X), torch.tensor(Y, dtype=torch.uint8)
...
def main():
    # 配置参数
    epoch_num = 10  # 训练轮数
    batch_size = 2000  # 每次训练样本个数
    train_sample = 5000000  # 每轮训练总共训练的样本总数
    input_size = 5  # 输入向量维度
    ...
    # 保存模型
    torch.save(model.state_dict(), "week2/model.pth")
    # 画图
    print(log)
...
def predict(model_path, input_vec):
    ...
    for vec, res in zip(input_vec, pred):
        print("输入：%s, 预测类别：%d, 概率值：%f" % (vec, torch.max(res, 0)[1], torch.max(res, 0)[0]))  # 打印结果
...
if __name__ == "__main__":
    ...
    predict("week2/model.pth", test_vec)
